...
def run(
    ...
    enable_mqtt=False,  # enable MQTT communication
    mqtt_broker="124.70.54.142",  # MQTT broker address
    mqtt_port=1883,  # MQTT broker port
    mqtt_topic="test_A",  # MQTT topic for publishing detection results
    enable_serial=False,  # enable serial communication
    serial_port="/dev/ttyAMA0",  # serial port (default for Raspberry Pi)
    serial_baud=115200,  # serial baud rate
    quantize_model=False,  # quantize model for Raspberry Pi optimization
):
    ...
    if enable_mqtt:
        try:
            ...
            if mqtt_client.connect():
                LOGGER.info(f"MQTT连接成功: {mqtt_broker}:{mqtt_port}, 主题: {mqtt_topic}")
            else:
                LOGGER.warning("MQTT连接失败，将禁用MQTT功能")
                mqtt_client = None
        except Exception as e:
...
def parse_opt():
    ...
    parser.add_argument("--view-img", action="store_true", default=True, help="show results (default: enabled)")
