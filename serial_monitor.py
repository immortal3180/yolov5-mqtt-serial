...
def monitor_serial(port, baudrate, timeout=1):
    ...
    try:
        ...
        if "set" in locals() and set.is_open:
            set.close()
            print("🔌 串口已关闭")
...
def send_test_data(port, baudrate):
    """发送测试数据（用于测试）."""
    try:
        set = serial.Serial(port, baudrate, timeout=1)
        print(f"✅ 串口连接成功: {port}")
...
使用示例:
  ...
  python serial_monitor.py --port /dev/ttyUSB0 --test
    ...
    parser.add_argument("--port", "-p", type=str, default="/dev/ttyAMA0", help="串口设备路径 (默认: /dev/ttyAMA0)")
    ...
    parser.add_argument("--test", "-t", action="store_true", help="发送测试数据")
